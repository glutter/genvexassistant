...
- Click a triangle or recent-event entry to zoom around its timestamp. History range, view and series selections are saved locally in the browser; absolute zoom windows and ventilation commands are not restored.
- Gaps in device measurements break chart lines, including in downsampled history. The rolling 24-hour overview counts only intervals between valid, sufficiently close observations. Missing time and the interval after the newest reading stay unobserved rather than being assigned a fan stage. Automatic increases count recorded automatic target changes, not RPM fluctuations or manual commands, and do not depend on the graph range.
- Temporary manual boost shows a live countdown and a stop button. Stopping releases the override and requests a fresh control evaluation without forcing a lower speed or discarding recovery/protection state. Monitor-only mode remains unchanged. The displayed fan stage continues to reflect measurements; acceptance of the stop request is not physical confirmation. Timer expiry waits for live confirmation too.
